...
export const updateCharacter = async (req, res) => {
    ...
    try {
        if (!mongoose.Types.ObjectId.isValid(updatedCharacter._id))
            return res.status(404).send(`No character with id: ${updatedCharacter.userId}`);
...
export const increaseStatistic = async (req, res) => {
    ...
    try {
        ...
        } else if (statistic === 'healthpoints') {
            updatedCharacter = await Character.findByIdAndUpdate( characterId, { $inc: { 'healthpoints': Number(value) } }, { new: true } );

            if (updatedCharacter.healthpoints > updatedCharacter.maxHealthpoints)
                updatedCharacter = await Character.findByIdAndUpdate( characterId, { 'healthpoints': updatedCharacter.maxHealthpoints }, { new: true } );

            return res.status(200).json({ result: updatedCharacter });
        } else if (statistic === 'experience') {
